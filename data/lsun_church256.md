...
中国大陆目标机优先使用 OpenDataLab 页面或平台 CLI 获取 LSUN，避免把 P1 建立在国外直连
下载可用的前提上。进入 OpenDataLab 的 LSUN 数据集页面，按页面给出的下载链接或 CLI
命令获取 Church Outdoor 训练集文件。OpenDataLab 的具体文件名和 CLI 数据集名可能调整，
因此以页面显示为准：
...
如果目标机无法使用 OpenDataLab CLI，就在本地浏览器或其他网络环境下载 Church Outdoor
训练集 LMDB/压缩包，再传到目标机并整理为：
