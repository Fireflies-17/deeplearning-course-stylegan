...
项目以固定的官方 StyleGAN2-ADA 实现为模型基础，不宣称从零重写网络或提出新架构。个人完成的
工程实现与实验工作包括：
...
| 课程要求 | 本项目对应工作 | 边界 |
|---|---|---|
| 至少实现一种 GAN 并完成图像生成 | 固定官方 StyleGAN2-ADA 后端，完成现代环境兼容补丁、配置驱动训练、生成和评价流程 | 不宣称从零重写网络 |
| 参数调优并分析关键因素 | E1-E5 比较 noaug、固定增强、ADA、数据规模和 ADA target，并增加关键条件的第二随机条件 | 单随机条件结果保持探索性 |
| 尝试改进现有方法 | 调整增强策略与 ADA target，并将单次 FID 扩展为多指标、失败案例、水印和最近邻审计 | 属于训练策略和评价流程改进，不是网络结构创新 |
| 提交可复现代码和 README | 固定后端提交、环境文件、补丁、预检、数据转换、统一入口、测试和证据清单 | 大型数据与快照需单独交付 |
...
详细下载、转换和溯源信息见 `data/lsun_church256.md`。目标机已确认：

- 转换 ZIP 大小：19,712,169,438 字节；
- 图像：100,000 张 256x256 RGB；
...
Shell 历史文件不存在，因此实际下载来源和日期无法恢复。文件修改时间不得写成下载日期。目标机
采集命令为：
...
采集器会计算 SHA-256/MD5、检查 ZIP 图像数量与分辨率、统计原始目录，并从 shell 历史提取候选
下载命令。URL 查询参数、令牌和认证头会被脱敏；文件修改时间只记录为文件时间，不当作下载日期。
...
| 组 | 总时长 | 最终 ADA p | 全程最大区间显存 | 末段区间显存 | 最后一段 sec/kimg |
|---|---:|---:|---:|---:|---:|
| E1 2000 | 7.35 h | 0.1531 | 17.66 GB | 3.24 GB | 10.94 |
| E2 | 4.56 h | 0 | 17.66 GB | 3.07 GB | 9.33 |
| E3 | 4.98 h | 0.2 | 17.66 GB | 3.16 GB | 9.98 |
| E4 | 5.05 h | 0.1336 | 17.66 GB | 3.15 GB | 9.98 |
| E5 | 5.04 h | 0.3758 | 17.66 GB | 3.17 GB | 9.68 |

这些时长对应单个双卡 run 的墙钟时间，不是四卡或六卡机器的总运行时长。显存为训练日志中
rank 0 单卡的区间峰值；后端每个 tick 后重置统计，因此全程最大值取所有区间的最大值，末段值
只描述稳定训练后期，不能替代全程峰值。
...
- 风格混合能够交换粗粒度结构与细粒度外观；
- W 空间插值连续，没有明显跳变；
- 88 张独立生成样本中人工确认 8 张带水印类文字，另有 1 张标为疑似；
- 151 帧插值中确认 46 帧连续出现水印类文字，2 帧为过渡疑似帧；
- 风格混合图显示污染纹理可以随风格源传播，但不计入独立样本比例；
- 当前 8 个 E1 生成样本的最近邻拼图没有观察到近乎像素复制的训练样本。

水印现象应称为“训练数据污染特征被模型学习和复现”，不能在缺少直接证据时称为模型记忆了某张
训练样本。插值帧彼此相关，不能与 88 张独立样本合并计算发生率。
...
- `inception-2015-12-05.pt`：FID、KID、最近邻；
...
`scripts/build_evidence.py` 从原始 metric JSONL、`stats.jsonl`、配置和人工标注生成并校验：

- `evidence/experiment_manifest.csv`：8 个正式条件的协议、资源、指标和来源；
- `evidence/claim_evidence_matrix.csv`：允许表述和限制条件；
- `evidence/visual/failure_cases.csv`：64 张 P2 样例逐张失败模式标注；
- `evidence/visual/watermark_annotations.csv`：88 张独立样本水印标注；
- `evidence/visual/interpolation_watermark_audit.csv`：151 帧插值轨迹水印标注；
- `evidence/nearest_neighbor_audit.md`：8 个生成样本、24 条近邻记录的核验。

## 测试状态

- 28/28 项不依赖 GPU 的单元测试通过；
- 25 个项目 Python 文件 AST 解析通过；
- 11 份 JSON 配置可解析；
...
1. LSUN 实际下载来源和日期因目标机 shell 历史不可用而无法恢复；
