...
当前实验、评估、主要图表和本地证据链已经完成，不需要继续追加训练。正式报告和最终提交整理
尚未开始；数据文件哈希等目标机元数据等待采集结果回传。具体状态见
[项目进度清单.md](项目进度清单.md)。
...
详细下载、转换和待回填信息见 `data/lsun_church256.md`。当前本地仍缺下载日期、实际来源、
原始数据大小、转换 ZIP 大小和校验值，不能凭空补写。目标机使用以下命令一次性采集：
...
1. LSUN 数据下载日期、实际来源、原始大小、ZIP 大小和校验值等待目标机采集结果回传；
